...
describe('ApiKeysController', () => {
  ...
  beforeEach(async () => {
    jest.clearAllMocks();
    const moduleRef: TestingModule = await Test.createTestingModule(moduleMetadata).overrideGuard(JwtAuthGuard).useValue(guardOverride).compile();
    controller = moduleRef.get<ApiKeysController>(ApiKeysController);
  });

  it('POST / returns one-time plaintext key in body', async () => {
    const expected: CreatedApiKey = { id: TEST_KEY_ID, label: 'Test', key: 'mk_abc123def456', keyPrefix: 'abcdefgh', createdAt: new Date() };
    mockApiKeysService.createForUser.mockResolvedValue(expected);
    const result = await controller.create({ user: { id: TEST_USER_ID } } as unknown as AuthedReq, { label: 'Test' });
    expect(result).toHaveProperty('key');
    expect(result.key).toMatch(/^mk_/);
  });

  it('GET / returns sanitized list without key field', async () => {
    const summaries: ApiKeySummary[] = [{ id: TEST_KEY_ID, label: 'Key 1', keyPrefix: 'abcdefgh', createdAt: new Date(), lastUsedAt: null, revokedAt: null }];
