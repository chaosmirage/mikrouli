...
test('revoked key returns 401', async ({ page, api }) => {
  ...
  // response wait must be armed BEFORE the click. Awaiting it keeps the next
  // API call from racing the revoke commit (flaky 201 instead of 401).
  const revokeResponse = page.waitForResponse(
    (resp) =>
      resp.url().includes(`/api/api-keys/${keyBody.id}`) && resp.request().method() === 'DELETE',
  );
  await page.getByTestId(`revoke-${keyBody.id}`).click();
  await revokeResponse;
  // The row's retired standing: the status reads Revoked once the refreshed
  // list lands, and the retire reach is spent (cannot fire again).
  await expect(page.getByTestId(`key-row-${keyBody.id}`)).toContainText(REVOKED_STANDING);
  await expect(page.getByTestId(`revoke-${keyBody.id}`)).toBeDisabled();
