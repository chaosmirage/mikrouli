...
test('editing a link updates the redirect target immediately, keeping the short URL and analytics', async ({
  page,
  api,
}) => {
  ...
  await page.getByTestId('edit-url-input').fill(NEW_URL);
  const editResponse = page.waitForResponse(
    (r) => r.url().includes(`/api/urls/${slug}`) && r.request().method() === 'PATCH',
  );
  await page.getByTestId('edit-confirm').click();
  await editResponse;
...
test('editing a link owned by another user is refused with 403', async ({ page, api }) => {
  ...
  const resp = await page.request.fetch(`http://localhost:8888/api/urls/${slug}`, {
    method: 'PATCH',
