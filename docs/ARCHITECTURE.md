...
is reached. The allowance defaults to a global value with an optional nullable
per-user override column on the user row (`apps/api/src/usage/usage.service.ts`).
